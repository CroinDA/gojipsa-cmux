# 꼬집사 (Pinchy) — Feature Roadmap

> Sentinel for cmux의 다음 단계 기능 확장 계획.
...
| 단계 | 정체성 | 한 줄 |
|------|-------|------|
| 현재 (v1.0.0) | Sentinel (파수꾼) | 위험한 명령 감지하는 관찰자 |
| 다음 | **꼬집사 (Pinchy)** | 옆자리에 앉아 같이 일하는 동료. 위험할 땐 꼬집고, 약속 시간엔 챙겨주고, 심심하면 말 걸어옴. |

**브랜딩 결정**:
- 프로젝트 기술명: `Sentinel` 유지 (국제 심사위원, GitHub 가시성)
- 캐릭터 페르소나: 한국어 **꼬집사** / 영어 **Pinchy**
...
| `~/.sentinel/memory.jsonl` | 최근 코멘트 + 사용자 반응 append-only 로그 | 신규 |
...
| `~/.sentinel/events.json` | 등록된 이벤트 (id, timestamp, label, fired) |
| `Scheduler.swift` | 30초마다 due 이벤트 체크, 타임아웃 시 onFire 콜백 |
| CLI 인터페이스 | `Sentinel remind "5분 후" "점심 먹기"` |
| 자연어 인터페이스 (보너스) | cmux 화면에 `# remind: 14시 미팅` 주석 → Sentinel이 자동 등록 |
...
1. **꼬집사 vs Sentinel 브랜딩**: 캐릭터명만 꼬집사? 프로젝트명도 변경?
